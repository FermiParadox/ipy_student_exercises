...
from main import SolveForX, solve_1rst_degree_poly, AnyNumber, NoSolution
...
class Test__hard_diff_left_and_right(TestCase):
    ...
    def test_n_x_terms(self):
        ...
        for _ in range(REPETITIONS // 100):
            l, r = SolveForX()._hard_diff_left_and_right(x_terms=n_x_terms, non_x_terms=3)
    ...
    def test_n_non_x_terms(self):
        n_non_x_terms = 6
        for _ in range(REPETITIONS // 100):
            l, r = SolveForX()._hard_diff_left_and_right(x_terms=3, non_x_terms=n_non_x_terms)
...
class Test_question(TestCase):
    def setUp(self):
        self.questions = set()
        for _ in range(REPETITIONS//100):
            for d in SolveForX.ALLOWED_DIFFICULTIES:
                inst = SolveForX(difficulty=d)
...
class Test_answer_types(TestCase):
    def setUp(self):
        ...
        for _ in range(REPETITIONS//100):
            for d in SolveForX.ALLOWED_DIFFICULTIES:
                inst = SolveForX(difficulty=d)
                self.answers.add(inst.answer)

    def test_answer_type_within_allowed(self):
        for a in self.answers:
            self.assertIsInstance(a, tuple(SolveForX().answer_types))
